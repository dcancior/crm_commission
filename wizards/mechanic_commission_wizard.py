...
class MechanicCommissionWizard(models.TransientModel):
    ...
    def _onchange_build_lines(self):
        ...
        for w in self:
            ...
            # Líneas de servicio del mecánico
            ...
            for line in inv_lines:
                ...
                porcentaje = float(line.product_id.product_tmpl_id.porcentaje_comision_mecanico or 0.0)
                subtotal = float(line.price_subtotal or 0.0)
                # Calcular comisión directamente (p.ej. 1000 * 50% = 500)
                ...
                vals_base = {
                    ...
                    'invoice_name': f"{line.move_id.name or ''} - {line.move_id.partner_id.display_name}",
                    'invoice_date': line.move_id.invoice_date,
                    'product_id': line.product_id.id,
                    'product_name': line.product_id.display_name,
                    'quantity': line.quantity or 0.0,
                    'subtotal_customer': subtotal,
                    'porcentaje_comision_mecanico': porcentaje,   # <-- % tal cual (50)
                    'commission_amount': commission,               # <-- monto ya calculado
            ...
            lines_cmds = [(5, 0, 0)]
            for entry in entries_to_keep.sorted(lambda r: (r.invoice_date or fields.Date.today(), r.id)):
                lines_cmds.append((0, 0, {'commission_entry_id': entry.id}))

            w.line_ids = lines_cmds
